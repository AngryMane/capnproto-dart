...
# Wait up to `attempts * 0.5` s (default 10 s) for a TCP port to accept
# connections.
wait_for_port() {
...
# Every in-tree Dart package that ships its own lib/ API and should be
# analyzed and unit-tested directly (as opposed to sample/test-only apps that
# only exercise these transitively).
DART_PACKAGES=(
...
for package in "${DART_PACKAGES[@]}"; do
  run_section "Dart unit tests: $package"
  if (cd "$package" && dart pub get && dart test); then
    pass "$package unit tests"
  else
    fail "$package unit tests"
  fi
done

# ── 1b. Benchmarks ───────────────────────────────────────────────────────────
#
# Not correctness tests — timings vary by machine, so these don't gate on
# specific numbers, only on the benchmark running to completion. Each script
# prints a JSON line plus a Markdown table, and appends the table to
# $GITHUB_STEP_SUMMARY when running in a GitHub Actions job, so results are
# visible on the workflow run's summary page without digging through logs.
BENCHMARKS=(
  "packages/capnproto_dart:benchmark/serialization_benchmark.dart"
  "packages/capnproto_dart_rpc:benchmark/rpc_benchmark.dart"
  "packages/capnproto_dart_rpc:benchmark/rpc_uds_benchmark.dart"
)

for entry in "${BENCHMARKS[@]}"; do
  package="${entry%%:*}"
  script="${entry#*:}"
  run_section "Benchmark: $package/$script"
  if (cd "$package" && dart pub get && dart run "$script"); then
    pass "$package/$script"
  else
    fail "$package/$script"
  fi
...
# AOT-compiled runs of the same scripts (`dart compile exe`), so the summary
# also shows Dart without JIT warmup/interpretation overhead — a closer
# comparison to Rust's own AOT-native binaries below. Each script's suite
# name gets a "[AOT]" suffix (passed as argv[0]) so JIT and AOT rows don't
# collide when both land in the same $GITHUB_STEP_SUMMARY.
BENCHMARK_AOT_DIR="$(mktemp -d)"
trap 'rm -rf "$BENCHMARK_AOT_DIR"' EXIT

for entry in "${BENCHMARKS[@]}"; do
  package="${entry%%:*}"
  script="${entry#*:}"
  aot_bin="$BENCHMARK_AOT_DIR/$(basename "$script" .dart)"
  run_section "Benchmark (AOT): $package/$script"
  if (cd "$package" && dart compile exe "$script" -o "$aot_bin") \
      && "$aot_bin" '[AOT]'; then
    pass "$package/$script (AOT)"
  else
    fail "$package/$script (AOT)"
  fi
done

trap - EXIT
rm -rf "$BENCHMARK_AOT_DIR"

# Rust counterparts of the same benchmarks (same schema, same iteration
# counts, same methodology — see test/interop/benchmark/rust/src/bin/) for a
# side-by-side reference point against the official Rust implementation.
# Not a judgment of this repo specifically — Dart (JIT/VM) vs Rust (AOT
# native) numbers reflect the languages' execution models as much as either
# implementation's quality.
RUST_BENCHMARKS=(serialization_benchmark rpc_benchmark rpc_uds_benchmark)

for bin in "${RUST_BENCHMARKS[@]}"; do
  run_section "Benchmark (Rust): test/interop/benchmark/rust/$bin"
  if cargo run --manifest-path test/interop/benchmark/rust/Cargo.toml --release --bin "$bin"; then
    pass "test/interop/benchmark/rust/$bin"
  else
    fail "test/interop/benchmark/rust/$bin"
  fi
done
...
if [[ -z "${goto_summary:-}" ]]; then
  ...
  if wait_for_port 127.0.0.1 12345; then
    if (cd sample/greeter/client && dart pub get && dart run build_runner build --delete-conflicting-outputs && dart run bin/main.dart); then
...
if [[ -z "${goto_summary2:-}" ]]; then
  ...
  if wait_for_port 127.0.0.1 12346; then
    if (cd test/interop/complex/client && dart pub get && dart run build_runner build --delete-conflicting-outputs && dart run bin/main.dart); then
...
if [[ -z "${goto_summary3:-}" ]]; then
  ...
  (cd test/interop/complex/dart-server && dart pub get && dart run build_runner build --delete-conflicting-outputs && dart run bin/main.dart) &
  DART_SERVER_PID=$!
  trap 'kill $DART_SERVER_PID 2>/dev/null || true' EXIT

  # A longer budget than the other wait_for_port calls: this dart-server
  # now runs `build_runner build` before `bin/main.dart` even starts, which
  # alone can take several seconds (longer still on a cold .dart_tool cache).
  if wait_for_port 127.0.0.1 12347 60; then
...
if [[ -z "${goto_summary4:-}" ]]; then
  ...
  if (cd test/interop/schema-evolution/dart && dart pub get && dart run build_runner build --delete-conflicting-outputs); then
...
if capnp compile -o- "$F08_TMP/new_compatible.capnp" \
    | dart run dev_packages/capnpc-dart/bin/capnpc_dart.dart --check="$F08_TMP/old.capnp"; then
...
if capnp compile -o- "$F08_TMP/new_iface_compatible.capnp" \
    | dart run dev_packages/capnpc-dart/bin/capnpc_dart.dart --check="$F08_TMP/old_iface.capnp"; then
...
(
  ...
  capnp compile -o- foo.capnp bar.capnp consts.capnp \
    | dart run "$REPO_ROOT/dev_packages/capnpc-dart/bin/capnpc_dart.dart"
)
...
if (cd test/interop/wire-format-golden/dart && dart pub get && dart run build_runner build --delete-conflicting-outputs); then
