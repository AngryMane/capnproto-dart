...
SchemaField _slotField(
  ...
  body: SlotField(
    offset: offset,
    type: type,
    hadExplicitDefault: defaultValue != null,
    defaultValue: defaultValue,
  ),
);

SchemaEnumerant _enumerant(String name, int codeOrder, {int? ordinal}) =>
    SchemaEnumerant(
      name: name,
      codeOrder: codeOrder,
      // Defaults to codeOrder, matching _field's convention above; tests
      // that specifically exercise the codeOrder-vs-ordinal distinction
      // pass ordinal explicitly.
      ordinal: ordinal ?? codeOrder,
    );
...
void main() {
  group('checkCompatibility — compatible changes', () {
    ...
    test('reordering enum variant declarations without changing @N '
        'ordinals is compatible', () {
      // Regression test mirroring the field-reordering test above, for
      // enums: `a` and `b` swap declaration order (codeOrder) between
      // old/new, but each keeps its own @N (ordinal) — a no-op on the wire.
      final old = _req([
        _enum([
          _enumerant('a', 0, ordinal: 0),
          _enumerant('b', 1, ordinal: 1),
        ]),
      ]);
      final newReq = _req([
        _enum([
          _enumerant('b', 0, ordinal: 1),
          _enumerant('a', 1, ordinal: 0),
        ]),
      ]);
      expect(checkCompatibility(old, newReq), isEmpty);
    });
  ...
  group('checkCompatibility — incompatible changes', () {
    ...
    test('changing an enum variant\'s actual @N ordinal is detected even '
        'when codeOrder stays the same', () {
      // Complements the reordering test above: codeOrder alone must not be
      // treated as compatible either — a real ordinal change (which does
      // change the enumerant's wire value) has to still be caught.
      final old = _req([
        _enum([_enumerant('a', 0, ordinal: 0)]),
      ]);
      final newReq = _req([
        _enum([_enumerant('a', 0, ordinal: 1)]),
      ]);
      expect(
        checkCompatibility(old, newReq),
        contains('MyEnum.a (ordinal 0) was removed'),
      );
    });

    test('changing a field\'s default value is incompatible', () {
      // The wire stores value XOR default, so a message written under the
      // old default silently decodes to a different value under the new
      // one — not a single byte needs to change for behavior to change.
      final old = _req([
        _struct([_slotField('x', 0, const Int32Type(), defaultValue: 5)]),
      ]);
      final newReq = _req([
        _struct([_slotField('x', 0, const Int32Type(), defaultValue: 7)]),
      ]);
      final errors = checkCompatibility(old, newReq);
      expect(errors, hasLength(1));
      expect(errors.first, contains('x'));
      expect(errors.first, contains('default value changed'));
    });

    test('changing a Data field\'s default value (byte-list) is '
        'incompatible', () {
      final old = _req([
        _struct([
          _slotField(
    ...
    test('a field moving between union membership/discriminant values is '
        'incompatible', () {
      final old = _req([
        _struct([_slotField('x', 0, const Int32Type())]), // not a union member
      ]);
      final newReq = _req([
        _struct([
          _slotField('x', 0, const Int32Type(), discriminantValue: 0),
        ]),
      ]);
      final errors = checkCompatibility(old, newReq);
      expect(errors, hasLength(1));
      expect(errors.first, contains('x'));
      expect(errors.first, contains('discriminant value changed'));
    });

    test('changing a union\'s discriminant offset is incompatible', () {
      final old = _req([
        _struct([
          _slotField('x', 0, const Int32Type(), discriminantValue: 0),
        ], discriminantCount: 1, discriminantOffset: 0),
      ]);
      final newReq = _req([
        _struct([
          _slotField('x', 0, const Int32Type(), discriminantValue: 0),
        ], discriminantCount: 1, discriminantOffset: 1),
      ]);
      final errors = checkCompatibility(old, newReq);
      expect(errors, hasLength(1));
      expect(errors.first, contains('discriminant offset changed'));
    });
