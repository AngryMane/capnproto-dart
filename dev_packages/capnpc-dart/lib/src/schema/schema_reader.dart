      ...
      if (el != null) {
        for (int i = 0; i < el.length; i++) {
          enumerants.add(SchemaEnumerant(
            ...
            // The compiler always emits Node.enum.enumerants ordered by wire
            // value (list position i == the enumerant's `@N`), regardless of
            // source declaration order — verified directly against `capnp
            // compile`'s actual CodeGeneratorRequest output for an enum
            // whose `@N`s are deliberately out of declaration order. `i` is
            // therefore the enumerant's real ordinal; codeOrder above is
            // only its textual declaration position.
            ordinal: i,
