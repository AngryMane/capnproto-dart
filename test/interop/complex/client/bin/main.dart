...
Future<void> _s01_codeGeneration() async {
  ...
  check(
    'echoScalars is method 1',
    true,
  ); // interface ordinal baked into dispatch

  // Nested types are accessible
  check('Person.Relationship enum exists', Relationship.values.isNotEmpty);
  checkEq(
    'Relationship.parent index',
    relationshipToUint16(Relationship.parent),
    0,
  );
  checkEq(
    'Relationship.other index',
    relationshipToUint16(Relationship.other),
    6,
  );
...
Future<void> _s02_allScalars(ComplexTestServiceClient svc) async {
  ...
  for (final pair in [
    ...
    final rr = await svc.echoScalars(
      (b) => b.initValue().float64Value = pair.$2,
    );
    checkEq(pair.$1, rr.value!.float64Value, pair.$2);
  }
  final rNaN = await svc.echoScalars(
    (b) => b.initValue().float64Value = double.nan,
  );
...
Future<void> _s04_nestedStructs(ComplexTestServiceClient svc) async {
  ...
  check(
    'partial struct email null-or-empty',
...
Future<void> _s07_genericStructs(ComplexTestServiceClient svc) async {
  ...
  final treeReader = MessageReader.deserialize(
    treeMsg.serialize(),
  ).getRoot(treeFactory);
  checkEq(
    'Tree<Person> root value',
    treeReader.value?.asStruct(personFactory)?.name,
    'TreeRoot',
  );
  checkEq(
  ...
  checkEq(
    'nested generic [2].key',
    r2.value!.people![0].attributes?[2].getTextField(0),
    'c',
  );
}
...
Future<void> _s08_recursive(ComplexTestServiceClient svc) async {
  ...
  checkEq(
    'recursive related rel',
    alice.related![0].relationship,
    Relationship.friend,
  );
  ...
  checkEq(
    'depth-2 grandchild',
    a.related![0].person?.related?[0].person?.name,
    'C',
  );
  ...
  checkEq(
    'wide tree [0]',
    r3.value!.people![0].related?[0].person?.name,
    'Child0',
  );
  checkEq(
    'wide tree [9]',
    r3.value!.people![0].related?[9].person?.name,
    'Child9',
  );
  ...
  final errorReader = MessageReader.deserialize(
    errorMsg.serialize(),
  ).getRoot(errorInfoFactory);
  ...
  checkEq(
    ...
    errorReader.cause?.some?.message,
    'root cause',
  );
}
...
Future<void> _s09_anyPointerRpc(ComplexTestServiceClient svc) async {
  ...
  check(
    'CapabilityFactory.getUntyped returns AnyPointer',
    untyped.value != null,
  );
  checkEq(
...
Future<void> _s10_basicRpc(ComplexTestServiceClient svc) async {
  ...
  final futures = List.generate(
    10,
    (i) => svc.echoScalars((b) => b.initValue().int32Value = i),
  );
...
Future<void> _s11_complexEcho(ComplexTestServiceClient svc) async {
  ...
  check(
    ...
    r.response?.status == Status.running ||
        r.response?.status == Status.unknown,
  ); // server may not set it
...
Future<void> _s14_capsInStructs(ComplexTestServiceClient svc) async {
  ...
  tgts14[0] = 0;
  tgts14[1] = 1;
  final reader14 = MessageReader.deserialize(
    mb14.serialize(),
  ).getRoot(capabilityBundleFactory, capabilities: [target14, target14b]);
  ...
  final optionalReader = MessageReader.deserialize(
    optionalMsg.serialize(),
  ).getRoot(capabilityBundleFactory, capabilities: optionalCaps);
  checkEq(
    'Capability in Optional which=some',
    optionalReader.optionalObserver?.which,
    1,
  );
  checkEq(
    'Capability in Optional cap index',
    optionalReader.optionalObserver?.someCapIndex,
    0,
  );
  check(
    'Capability in Optional typed cap',
  ...
  final noneReader = MessageReader.deserialize(
    noneMsg.serialize(),
  ).getRoot(capabilityBundleFactory);
  checkEq(
    'Optional capability none which',
    noneReader.optionalObserver?.which,
    0,
  );
  check(
    'Optional capability none has no cap',
  ...
  try {
    ...
    final ping0 = await t0!.ping((b) => b.payload = Uint8List.fromList([0x1A]));
    checkEq('echoed target[0].ping()', ping0.payload?[0], 0x1A);
    final ping1 = await t1!.ping((b) => b.payload = Uint8List.fromList([0x2B]));
...
Future<void> _s15_pipelining(ComplexTestServiceClient svc) async {
  ...
  final pingR15 = await target15.ping(
    (b) => b.payload = Uint8List.fromList([42]),
  );
  ...
  final childPing = await child1.ping(
    (b) => b.payload = Uint8List.fromList([1, 2]),
  );
  ...
  final deepPing = await child2.ping(
    (b) => b.payload = Uint8List.fromList([99]),
  );
...
Future<void> _s17_genericMethods(ComplexTestServiceClient svc) async {
  ...
  check(
    'CapabilityFactory.echoCapability<T> returned typed cap',
    echoedTarget != null,
  );
  ...
  checkEq(
    'CapabilityFactory.echoCapability<T> returned cap callable',
    ping.payload?[0],
    17,
  );
  ...
  final echoedTextList = await svc.echoAnyPointerTyped(textListCodec, [
    'alpha',
    'beta',
    'gamma',
  ]);
...
Future<void> _s21_streaming(ComplexTestServiceClient svc) async {
  ...
  try {
    ...
    checkEq(
      'ByteSource.pumpTo collected text',
      String.fromCharCodes(collectingSink.data),
      'test-resource',
    );
...
Future<void> _s23_nullValues(ComplexTestServiceClient svc) async {
  ...
  check(
    'unset data field',
    r3.value!.dataValue == null || r3.value!.dataValue!.isNotEmpty,
  ); // may have default
  ...
  check(
    'empty text != null text (different)',
...
Future<void> _s24_segmentation(ComplexTestServiceClient svc) async {
  ...
  try {
    final r4 = await svc.echoScalars(
      (b) => b.initValue().dataValue = largeData,
    );
  ...
  final r6 = await svc
      .probePipelineTarget((b) => b.payload = largeData, target: localTarget)
...
Future<void> _s27_concurrency(ComplexTestServiceClient svc) async {
  ...
  for (int round = 0; round < 3; round++) {
    final batchFuts = List.generate(
      20,
      (i) => svc.echoScalars((b) => b.initValue().int32Value = round * 100 + i),
    );
...
Future<void> _s28_resourceManagement(
  ...
  final alive = await target28a.ping(
    (b) => b.payload = Uint8List.fromList([1]),
  );
  ...
  try {
    ...
    check(
      'calling disposed cap fails locally',
      e.toString().contains('capability is disposed'),
    );
  }
  final afterDisposedCall = await svc.echoScalars(
    (b) => b.initValue().int32Value = 2802,
  );
  checkEq(
    'svc alive after disposed cap call',
    afterDisposedCall.value?.int32Value,
    2802,
  );
