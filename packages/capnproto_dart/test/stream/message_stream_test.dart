...
void main() {
  ...
  group('MessageStream.deserializeStream', () {
    ...
    test('reads many messages across chunk boundaries that don\'t align '
        'with message boundaries — stresses the internal buffer\'s '
        'consumed-bytes bookkeeping across both "several messages land in '
        'one chunk" and "one message spans several chunks"', () async {
      final pts = [for (var i = 0; i < 200; i++) (i, i * 2)];
      final concat = Uint8List.fromList(
        pts.expand((p) => _buildPoint(p.$1, p.$2)).toList(),
      );

      // 17 bytes doesn't evenly divide a Point message's frame size, so
      // chunk boundaries drift relative to message boundaries throughout.
      final results = await _deserializeChunked(concat, 17);
