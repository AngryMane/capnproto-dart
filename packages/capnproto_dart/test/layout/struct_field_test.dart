...
void main() {
  ...
  group('StructReader pointer section — Text fields', () {
    ...
    test('long all-ASCII text (cross-segment) survives round-trip', () {
      // Larger than MessageBuilder's default first-segment capacity, so
      // this exercises writeTextField's cross-segment path — regression
      // test for the ASCII fast path in _encodeUtf8 (arena_builder.dart),
      // which must produce byte-identical output to utf8.encode for
      // pure-ASCII input.
      final text = 'A' * 65536;
      expect(
        _roundTrip(withPtrsFactory, (b) => b.name = text, (r) => r.name),
    ...
    test(
      'long text with a non-ASCII code unit at the very start '
      'survives round-trip',
      () {
        final text = 'こ${'A' * 65535}';
        expect(
          _roundTrip(withPtrsFactory, (b) => b.name = text, (r) => r.name),
          equals(text),
        );
    ...
    test(
      'long text with a non-ASCII code unit in the middle survives '
      'round-trip',
      () {
        final text = '${'A' * 32768}こ${'A' * 32767}';
        expect(
          _roundTrip(withPtrsFactory, (b) => b.name = text, (r) => r.name),
          equals(text),
        );
    ...
    test(
      'long text with a non-ASCII code unit at the very end survives '
      'round-trip — the worst case for the ASCII fast path\'s early exit',
      () {
        final text = '${'A' * 65535}こ';
        expect(
          _roundTrip(withPtrsFactory, (b) => b.name = text, (r) => r.name),
          equals(text),
        );
    ...
    test('text with a surrogate-pair emoji survives round-trip', () {
      final text = '${'A' * 100}🎉${'A' * 100}';
      expect(
        _roundTrip(withPtrsFactory, (b) => b.name = text, (r) => r.name),
        equals(text),
      );
    ...
    test(
      'text with a lone (unpaired) surrogate defers to utf8.encode, same '
      'as before the ASCII fast path',
      () {
        // Not valid UTF-16 text, but Dart Strings don't reject it. The
        // first code unit (0x41) is ASCII so the fast path starts, but must
        // bail to utf8.encode as soon as it reaches the lone surrogate
        // (0xD800, >= 0x80) rather than mis-encoding it — same outcome as
        // calling utf8.encode directly (which itself lossily substitutes
        // U+FFFD for the unpaired surrogate; that substitution is
        // dart:convert's existing, unchanged behavior, not something this
        // fast path introduces).
        final text = String.fromCharCodes([0x41, 0xD800, 0x42]);
        final expected = utf8.decode(utf8.encode(text), allowMalformed: true);
        expect(
          _roundTrip(withPtrsFactory, (b) => b.name = text, (r) => r.name),
          equals(expected),
        );
