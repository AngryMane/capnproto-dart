...
void main() {
  ...
  group('resolveTextAt NUL terminator validation (RUNTIME-003)', () {
    ...
    test('invalid UTF-8 bytes throw DecodeException', () {
      // 0xFF is not valid UTF-8; followed by NUL so it passes our checks.
      // Malformed input must surface as DecodeException, not the raw
      // FormatException utf8.decode() throws internally, so callers only
      // need to catch one exception type for any kind of decode failure.
      expect(
    ...
    test(
      'invalid UTF-8 byte after a long ASCII prefix throws DecodeException',
      () {
        // Regression test for _decodeUtf8's ASCII fast path
        // (arena_reader.dart): a long run of ASCII bytes must not cause the
        // trailing invalid byte to be missed — the fast path has to keep
        // scanning through the whole ASCII prefix and still fall back to
        // utf8.decode (which rejects this) rather than stopping early.
        final bytes = [
          for (var i = 0; i < 1000; i++) 0x41, // 'A' * 1000
          0xFF, // invalid continuation byte
          0x00, // NUL terminator
        ];
        expect(
          () => resolveText(buildTextMessage(bytes)),
          throwsA(isA<DecodeException>()),
        );
      },
    );
