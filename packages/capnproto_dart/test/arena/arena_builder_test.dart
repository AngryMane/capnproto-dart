...
void main() {
  ...
  group('ArenaBuilder.allocate', () {
    ...
    test('overflow triggers a new segment', () {
      // Explicit small capacity so this test exhausts segment 0 exactly,
      // independent of whatever ArenaBuilder's default segment size is.
      final arena = ArenaBuilder(4);
      arena.allocate(4); // exhausts segment 0 exactly
  ...
  group('ArenaBuilder.serialize (framing format)', () {
    ...
    test('two-segment message has padding word', () {
      // Explicit small capacity so segment 0 is exhausted exactly,
      // independent of whatever ArenaBuilder's default segment size is —
      // only each segment's *used* word count (not its allocated capacity)
      // affects serialize()'s output, so this doesn't need to know or care
      // how large segment 1's capacity ends up being.
      final arena = ArenaBuilder(4);
      arena.allocate(4); // fill segment 0 exactly
      ...
      // Segment 0 contributes 4 used words, segment 1 contributes 1.
      expect(
        bytes.lengthInBytes,
        equals(headerBytes + (4 + 1) * bytesPerWord),
      );
    });
