...
class ContainerReader extends StructReader {
  ContainerReader(super.raw, {super.capabilities});

  String? get text => getTextField(0);
  ListReader<int>? get numbers => getUint32ListField(1);
  ContainerReader? get child => getStructFieldWith(
    2,
    (raw) => ContainerReader(raw, capabilities: capabilityTable),
  );
  int get capIndex => getCapabilityField(3);
}

class ContainerBuilder extends StructBuilder {
  ContainerBuilder(super.raw);

  set text(String value) => setTextField(0, value);
  ListBuilder<int> initNumbers(int count) => initUint32ListField(1, count);
  ContainerBuilder initChild() =>
      initStructFieldWith(2, ContainerBuilder.new, 0, 4);
  set capIndex(int value) => setCapabilityField(3, value);
...
class _ContainerFactory
  ...
  @override
  ContainerReader fromRawReader(RawStructReader raw) => ContainerReader(raw);

  @override
  ContainerReader fromRawReaderWithCapabilities(
    RawStructReader raw,
    List<Object?> capabilities,
  ) => ContainerReader(raw, capabilities: capabilities);

  @override
  ContainerBuilder fromRawBuilder(RawStructBuilder raw) =>
      ContainerBuilder(raw);
}
...
void main() {
  group('bytes-only message copy', () {
    ...
    test(
      'AnyPointerReader.asStruct reads text/list/nested-struct/capability '
      'fields in place, matching asDynamicStruct',
      () {
        final hostBuilder = MessageBuilder();
        final root = hostBuilder
            .initRoot(anyHostFactory)
            .initAnyPayload()
            .initStruct(containerFactory);
        root.text = 'hello';
        root.initNumbers(3)
          ..[0] = 1
          ..[1] = 2
          ..[2] = 3;
        root.initChild().text = 'child';
        root.capIndex = 0;
        ...
        final typed = any.asStruct(containerFactory)!;
        expect(typed.text, equals('hello'));
        expect(typed.numbers, orderedEquals([1, 2, 3]));
        expect(typed.child!.text, equals('child'));
        expect(typed.capIndex, equals(0));

        final dynamic_ = any.asDynamicStruct()!;
        expect(dynamic_.getTextField(0), equals(typed.text));
        expect(dynamic_.getUint32ListField(1), orderedEquals(typed.numbers!));
      },
    );

    test(
      'AnyPointerReader.asStruct shares the parent nesting-limit budget, '
      'same as asDynamicStruct',
      () {
        const chainLength = 5;
        final builder = MessageBuilder();
        var current = builder.initRoot(anyHostFactory);
        for (var i = 0; i < chainLength; i++) {
          current = current.initAnyPayload().initStruct(anyHostFactory);
        }

        // nestingLimit=1 leaves no budget to resolve even the first struct
        // in the chain, past the root itself.
        final reader = MessageReader.deserialize(
          builder.serialize(),
          const MessageReaderOptions(nestingLimit: 1),
        ).getRoot(anyHostFactory);

        expect(
          () => reader.anyPayload!.asStruct(anyHostFactory),
          throwsA(isA<DecodeException>()),
        );
        expect(
          () => reader.anyPayload!.asDynamicStruct(),
          throwsA(isA<DecodeException>()),
        );
      },
    );
