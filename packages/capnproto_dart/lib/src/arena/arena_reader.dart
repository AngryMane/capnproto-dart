...
import 'arena_builder.dart' show ArenaBuilder, RawStructBuilder;
...
class RawListReader {
  ...
  /// Holds the public [nestingLimit] value.
  ...
  /// Creates a [RawListReader] instance.
  const RawListReader({
...
class RawStructReader {
  ...
  /// Creates a [RawStructReader] instance.
...
class ArenaReader {
  ...
  /// final result = reader.getSegment(0);
  /// ```
  ...
  /// Performs the [chargeTraversal] operation.
  ///
  /// **Example**
  /// ```dart
  /// // Given the required message, schema, or raw-layout values:
  /// final operation = reader.chargeTraversal;
  /// ```
  ...
  /// ```dart
  /// // Given the required message, schema, or raw-layout values:
  /// final operation = reader.resolveStructAt;
  /// ```
  ...
  RawStructReader _resolveStructAt(
    ...
    throw DecodeException('expected struct pointer, got ${ptr.runtimeType}');
  }
  ...
  RawStructReader _resolveFarStruct(FarPointer far, int nestingLimit) {
    ...
    if (!dataSeg.containsInterval(
      dataWordOffset,
      tag.dataWords + tag.ptrWords,
    )) {
  ...
  /// ```dart
  /// // Given the required message, schema, or raw-layout values:
  /// final operation = reader.resolveListAt;
  /// ```
  ...
  RawListReader _resolveListPointerAt(
    ...
    if (ptr is! ListPointer) {
      throw DecodeException('expected list pointer, got ${ptr.runtimeType}');
    }

    return _buildListReaderDirect(
      seg,
      wordOffset + 1 + ptr.offset,
      ptr,
      nestingLimit,
    );
  }
  ...
  /// ```dart
  /// // Given the required message, schema, or raw-layout values:
  /// final operation = reader.resolveOptionalStructAt;
  /// ```
  ...
  /// ```dart
  /// // Given the required message, schema, or raw-layout values:
  /// final operation = reader.resolveTextAt;
  /// ```
  String? resolveTextAt(SegmentReader seg, int wordOffset) {
    final ptr = WirePointer.decode(seg.data, wordOffset);
    if (ptr is NullPointer) return null;
    final (listSeg, dataByteOffset, elementCount) = _resolveByteListPointer(
      seg,
      wordOffset,
      ptr,
    );
    ...
    final nulOffset =
        listSeg.data.offsetInBytes + dataByteOffset + elementCount - 1;
    ...
    try {
      ...
      throw DecodeException('Text field is not valid UTF-8', cause: e);
    }
  }

  // Mirrors _encodeUtf8 in arena_builder.dart: ASCII bytes are valid UTF-8
  // that decode to themselves 1:1, so this skips dart:convert's general
  // multi-byte-aware decoder for that case (measured via CPU profiling to
  // be a dominant cost for large Text fields, alongside the encode side).
  // Falls back to utf8.decode — unchanged behavior, including throwing
  // FormatException on invalid sequences — at the first non-ASCII byte.
  ...
  /// ```dart
  /// // Given the required message, schema, or raw-layout values:
  /// final operation = reader.resolveDataAt;
  /// ```
  ...
  /// ```dart
  /// // Given the required message, schema, or raw-layout values:
  /// final operation = reader.fromBytes;
  /// ```
  static ArenaReader fromBytes(Uint8List bytes, MessageReaderOptions options) {
...
/// Produces a [RawStructReader] sharing the same underlying buffer as
/// [builder] — no copy, no serialize/re-parse. Subsequent writes through
/// [builder] remain visible through the returned reader.
///
/// Shared by [StructBuilder.rawToReader] and [AnyPointerBuilder.asReader]
/// (both just wrap this), and usable directly by callers holding a raw
/// [RawStructBuilder] with no typed wrapper.
RawStructReader rawStructBuilderToReader(RawStructBuilder builder) {
  final arena = ArenaReader.fromBuilder(builder.arena);
  return RawStructReader(
    segment: arena.getSegment(builder.segment.id),
    arena: arena,
    dataWordOffset: builder.dataWordOffset,
    dataWords: builder.dataWords,
    ptrWordOffset: builder.ptrWordOffset,
    ptrWords: builder.ptrWords,
    nestingLimit: 64,
  );
}
