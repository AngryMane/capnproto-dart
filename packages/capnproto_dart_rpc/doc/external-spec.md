...
Implements **Cap'n Proto RPC Level 1** for two-party connections: object-capability
references, promise pipelining, tail calls, and Resolve/Disembargo in both
directions. Per `rpc.capnp`'s own level definitions, none of that requires
three-party handoff — three-party handoff (`Provide`/`Accept`, `ThirdPartyCapDescriptor`)
is Level 3, and persistent capabilities (`Save`/`Restore`) are Level 2; both are
separate, higher levels, not part of Level 1 itself, and are out of scope here.
Level 4 (`Join`, reference-equality checking) is out of scope too.

## Unsupported capability descriptors

Receiving an unsupported capability descriptor, including Level 3
...
reference counts. Peers that require three-party handoff or persistent-capability
descriptors are therefore not interoperable with this Level 1 implementation and
must negotiate or use a different connection.
## Core RPC types
