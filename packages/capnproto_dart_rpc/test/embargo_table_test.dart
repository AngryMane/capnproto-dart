...
void main() {
  group('EmbargoTable', () {
    test('register then resolve completes the completer and drops tracking', () {
    ...
    test('resolve arriving before the timeout cancels it: the completer '
        'settles successfully and a late timer firing never overwrites that',
        () async {
      final table = EmbargoTable();
      final completer = Completer<void>();
      final id = table.register(completer, timeout: const Duration(milliseconds: 30));
    ...
    test('timeout firing before resolve() fails the completer, and a later '
        'resolve() call for the same (now-expired) id is a safe no-op',
        () async {
      final table = EmbargoTable();
      final completer = Completer<void>();
      final id = table.register(completer, timeout: const Duration(milliseconds: 20));
    ...
    test('tearDown fails every still-pending embargo and clears the table', () async {
      final table = EmbargoTable();
    ...
    test('tearDown does not touch an embargo already resolved before it ran', () async {
      final table = EmbargoTable();
      final completer = Completer<void>();
      final id = table.register(completer);
      table.resolve(id);
