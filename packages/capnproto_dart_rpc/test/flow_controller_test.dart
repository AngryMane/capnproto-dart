...
void main() {
  group('StreamingCallFlowController', () {
    test('trackCall resolves immediately while under the window', () async {
      final fc = StreamingCallFlowController(windowSize: 100);
      final ack = Completer<void>();
      var resolved = false;
      unawaited(fc.trackCall(10, ack.future).then((_) => resolved = true));
      await Future<void>.delayed(Duration.zero);
      expect(resolved, isTrue);
      expect(fc.debugInFlight, equals(10));
    });

    test('trackCall blocks once in-flight bytes reach the window, and resolves '
        'once an earlier call is acked', () async {
      // The window is extended by the largest message seen so far (see
      // StreamingCallFlowController doc comment), so use uniform message sizes here —
      ...
      // blocked).
      final fc = StreamingCallFlowController(windowSize: 10);
      final ack1 = Completer<void>();
      final ack2 = Completer<void>();
      final ack3 = Completer<void>();

      var firstResolved = false;
      unawaited(fc.trackCall(5, ack1.future).then((_) => firstResolved = true));
      var secondResolved = false;
      unawaited(
        fc.trackCall(5, ack2.future).then((_) => secondResolved = true),
      );
      await Future<void>.delayed(Duration.zero);
      ...
      unawaited(fc.trackCall(5, ack3.future).then((_) => thirdResolved = true));
      await Future<void>.delayed(Duration.zero);
      expect(
        thirdResolved,
        isFalse,
    ...
    test('a message larger than the window is still sendable, and the window '
        'is extended by its size to avoid permanently stalling', () async {
      final fc = StreamingCallFlowController(windowSize: 10);
      final bigAck = Completer<void>();
      var bigResolved = false;
      unawaited(
        fc.trackCall(50, bigAck.future).then((_) => bigResolved = true),
      );
      await Future<void>.delayed(Duration.zero);
      ...
      unawaited(
        fc.trackCall(5, smallAck.future).then((_) => smallResolved = true),
      );
      await Future<void>.delayed(Duration.zero);
    ...
    test(
      'a failed ack fails currently-blocked calls and poisons future calls',
      () async {
        final fc = StreamingCallFlowController(windowSize: 5);
        final ack1 = Completer<void>();
        unawaited(fc.trackCall(5, ack1.future));
        await Future<void>.delayed(Duration.zero);
        ...
        final blockedResult = fc.trackCall(5, Completer<void>().future);
        ...
        await expectLater(
          fc.trackCall(1, Future<void>.value()),
